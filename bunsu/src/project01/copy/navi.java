...
public class navi {//
	...
	public void navigation() {
		...
		int fm=0,p,fsum = 0,nsum = 0,min,esum[]=new int[23];//1회용,각종 선택지,총거리,현제거리,임시거리,최소거리
		int n,z,count=0;//현제위치인 배열 이전위치 배열 끝 골목 배열인덱스,count=지나간거리수
		
		in.backup();
		dt.dotinfom();
		while(choo) {
		System.out.println("||1: 번호로 검색 |2: 한글로 검색 \n||3: 의료기구,응급센터 번호확인\n||4: 종료");
		System.out.print("선택>");
		p=scan.nextInt();
		if(p==1) {
		...
		for(int i=0;i<in.fcl.length;i++) {//끝점에 가장가까운 골목길 설정
			if(in.fcl[i].num==end) {//끝지점과 같은 장소배열 찾기
				for(int j=0;j<dt.dote.length;j++) {//각 골목길과의 거리비교를 위해 배열에 우겨넣기
					if(dt.dote[j]==null) {
